...
class SimConfig:
    ...
    price_process: str = "gbm"  # gbm|jump
    jump_intensity: float = 0.15  # expected jumps per year
...
def _jump_diffusion_paths(
    ...
    jump_sizes = rng.normal(jump_mean, jump_vol, size=(n_paths, n_steps)) * n_jumps
    drift_adj = mu - 0.5 * sigma * sigma - jump_intensity * (np.exp(jump_mean + 0.5 * jump_vol**2) - 1.0)
    increments = drift_adj * dt + sigma * np.sqrt(dt) * z + jump_sizes
    log_s = np.log(s0) + np.cumsum(increments, axis=1)
    s = np.concatenate([np.full((n_paths, 1), s0), np.exp(log_s)], axis=1)
    return s
