...
def _adjust_design(design: ExperimentDesign, summary: dict, *, iteration: int) -> tuple[ExperimentDesign, str]:
    ...
    if math.isnan(median_ret) or median_ret < metrics.get("target_median_return", STOP_CRITERIA["target_median_return"]):
        calibration_method = "iv_max" if calibration_method != "iv_max" else "iv_std"
        metrics["target_median_return"] = min(metrics.get("target_median_return", 0.01) + 0.002, 0.02)
        notes.append("raise_premium_and_target")

    if math.isnan(p_loss) or math.isnan(q05) or p_loss > metrics.get("max_loss_prob", STOP_CRITERIA["max_loss_prob"]):
        price_process = "jump_diffusion_proxy"
        validation_method = "monthly_backtest_with_stress"
        metrics["max_loss_prob"] = max(metrics.get("max_loss_prob", 0.25) - 0.02, 0.10)
        metrics["min_q01"] = max(metrics.get("min_q01", -0.20), -0.12)
        notes.append("tighten_tail_metrics")

    if validation_method == "monthly_backtest_with_stress":
        metrics["max_p_return_below_1pct"] = max(metrics.get("max_p_return_below_1pct", 0.55) - 0.05, 0.30)
        notes.append("stress_validation")

    if iteration % 2 == 1:
        price_process = PRICE_PROCESS_OPTIONS[(PRICE_PROCESS_OPTIONS.index(price_process) + 1) % len(PRICE_PROCESS_OPTIONS)]
    if iteration % 3 == 2:
        calibration_method = CALIBRATION_OPTIONS[(CALIBRATION_OPTIONS.index(calibration_method) + 1) % len(CALIBRATION_OPTIONS)]
    if iteration % 4 == 3:
        validation_method = VALIDATION_OPTIONS[(VALIDATION_OPTIONS.index(validation_method) + 1) % len(VALIDATION_OPTIONS)]
        notes.append("rotation_validation")
