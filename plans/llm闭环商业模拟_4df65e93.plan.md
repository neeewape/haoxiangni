---
name: LLM闭环商业模拟
overview: 用LLM生成/改进策略参数与模型假设，用可重复的仿真与检验模块产出收益、回撤与约束达标性，并由LLM基于反馈自动迭代直到满足月度+1%目标或触发停机条件。
todos:
  - id: define-metrics
  ...
  - id: assumption-tests
    ...
    dependencies:
      ...
      - sim-engine
  - id: llm-loop
    content: 实现LLM闭环：生成候选参数→运行评估→结构化反馈→自动修正，直至达标/收敛/预算停机。
    status: pending
    dependencies:
      - sim-engine
      - assumption-tests
...
1. **LLM生成候选方案**：给出一套参数 + 明确“为什么这样设”
2. **构建/迭代仿真系统**：把方案参数化落地到同一套模拟引擎
3. **测试与验证**：
...
4. **分析反馈**：输出“指标是否达标 + 失败分解 + 下一步改哪里”
5. **LLM修正/补全**：更新参数或模型假设，进入下一轮
